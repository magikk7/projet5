...
// console.log(my_url.has("name"));//true
    ...
    requete.onreadystatechange = function(){
        ...
        if (this.readyState == 4 && this.status == 200){
            ...
            //partie HTML - visible sur le site
            ...
            <div class="card-body card-center">
                ...
                <div class="caracteristique-dimension">
                        <p id="${this.response.name}" class="card-title2">${this.response.name} : <span id="price">${numStr(resultat,"")} &euro;</span></p>
                        <hr>
                        <form class="choix-dimension">
                            <label for="meubles">Choisir une couleur de vernis :</label>
                                <select class="form-select" name="meubles" id="meubles"></select>
                            <div class="d-grid gap-2">
                        <button class="btn btn-primary" id="bouton_validation" type="button">Ajouter au panier</button>
                        </form>
                        </div>
                    </div>
                </div>
    ...
    <div id="selection-produit" class="card">
        <div class="card-body" style="display: flex;>
            ...
            <div>
                <h3 id="${this.response.name}" class="card-title">${this.response.name}</h3>
                <p class="prix">Prix : <span id="${this.response.price}">${numStr(resultat,"")} &euro;</span></p>
                <a href="article.html?name=${this.response.produit._id}" id="${this.response.produit._id}" class="btn btn-gradient mt-4">Voir la description</a>
            </div>
        </div
    </div>  
    `
        ...
        //activer bouton "valider" / Incrémenter PANIER dans nav / Aller au PANIER
        ...
        idBouton.addEventListener('click', (event) => {
            event.preventDefault();//eviter le rechargement auto
            ...
            // console.log(infoLocalStorage);//null / rien

            const localstorage = function(){
                infoLocalStorage.push(choixMeuble);
                localStorage.setItem('caracteristiques', JSON.stringify(infoLocalStorage));
                idPanier.innerHTML = Number(infoLocalStorage.length);
                alert("Votre produit est enregistré dans votre panier");//violation...
            };
